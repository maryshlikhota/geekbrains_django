...
def runserver(*args):
    local(f'cd server; python manage.py runserver {" ".join(args)}')


def startapp(*args):
    local(f'cd server python manage.py startapp {" ".join(args)}')


def makemigrations(*args):
    local(f'cd server python manage.py makemigrations {" ".join(args)}')


def migrate(*args):
    local(f'cd server python manage.py migrate {" ".join(args)}')


def collectstatic(*args):
    local(f'cd server python manage.py collectstatic {" ".join(args)}')


def createsuperuser(*args):
    local(f'cd server python manage.py createsuperuser {" ".join(args)}')
